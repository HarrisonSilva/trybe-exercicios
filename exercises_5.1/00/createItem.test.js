...
describe('a função createItem', () => {
  it('cria um item válido', () => {
    const executeFunction = createItem('Video Game', 1, 500, 1)
    expect(executeFunction).toEqual({ name: 'Video Game', quantity: 1, unit: 1, price: 500 })
  });
  it('utiliza zero como quantidade padrão', () => {
    const executeFunction = createItem('Video Game', 1, 500,  );
    expect(executeFunction).toEqual({ name: 'Video Game', quantity: 0, unit: 1, price: 500 })
  });
  it('Lança um erro quando não recebe parâmetros', () => {
    expect(() => createItem()).toThrow();
  });
  it('Lança um erro se o nome do item não é uma string', () => {
    expect(() => createItem(1, 1, 500, 1)).toThrow();
  });
  it('Lança um erro se o preço é negativo', () => {
    expect(() => createItem(1, 1, -500, 1)).toThrow();
  });
  it('Lança um erro se o preço é zero', () => {
    expect(() => createItem(1, 1, 0, 1)).toThrow();
  });
});
